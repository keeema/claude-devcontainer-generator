...
CLI nástroj pro generování izolovaných devcontainer repozitářů s Claude Code sandbox.

## Proč?

Chceš používat Claude Code na zákaznickém projektu, ale nechceš:
- Přidávat `.devcontainer/` do zákaznického repa
- Sdílet svůj CLAUDE.md, poznámky a tooling se zákazníkem
- Řešit cleanup při ukončení spolupráce

Řešení: vygeneruj si **oddělené devcontainer repo**, které žije vedle zákaznického repa. Zákaznické repo zůstane čisté — žádné `.devcontainer/`, žádné `.claude/`.
...
```bash
node src/cli.js \
  --repo git@github.com:zakaznik/mujprojekt.git \
  --stack nodejs \
  --services postgres \
...
1. Naklonuje zákaznické repo vedle devcontaineru
...
~/projects/
  ...
  │   │   └── init.sh              ← klonuje zákaznické repo, vytvoří volumes
  │   └── project.yml              ← metadata projektu (repo URL, branch)
  └── mujprojekt/                  ← zákaznické repo (naklonované automaticky)
```
...
| Bind mount | `/workspace` | Zákaznický zdrojový kód | — |
...
|----------|---------|---------|-------|
| `--repo` | ano | — | Git URL zákaznického repa |
...
| `--include-compose` | ne | `false` | Zahrne zákaznický `docker-compose.yml` přes Docker Compose `include` |
...
```bash
node src/cli.js \
  --repo git@github.com:zakaznik/eshop.git \
  --stack nodejs \
  --services postgres,redis \
  --output ~/projects/eshop-devcontainer
```

### .NET projekt s plným internetem a zákaznickým docker-compose

```bash
node src/cli.js \
  --repo git@github.com:zakaznik/erp.git \
  --stack dotnet \
  --full-internet \
  --include-compose \
...
```bash
node src/cli.js \
  --repo git@github.com:zakaznik/ml-pipeline.git \
  --stack python \
...
Soubor přežije rebuild kontejneru a zákaznické repo zůstane čisté (volume překryje adresář).
